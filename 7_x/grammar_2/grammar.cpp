...
template<class Out>
void gen_aux_sentence(const Grammar& grammar, const string& word, Out o){
	if(!is_bracketed(word)){
		*o++ = word;
	} else {
		Grammar::const_iterator iter = grammar.find(word);
...
template <class Out>
void gen_sentence(const Grammar& grammar, Out o){
	gen_aux_sentence(grammar, "<sentence>", o);
}
